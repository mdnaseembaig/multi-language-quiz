...
export default function App() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    const unique = [
      ...new Set(
        (data || []).map((r: { category: string }) => r.category)
      ),
    ].sort();
          ...
          {totalPages > 1 && (
            <div className="mt-8 flex items-center justify-center gap-4">
              <button
                ...
                disabled={page === 0}
                className="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
                Previous
              </button>
...
function QuestionCard({
  ...
  return (
    <div className="group rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
      ...
      <p className="mb-4 text-sm font-semibold leading-relaxed text-slate-800">
        {question.question}
      </p>
...
function QuizView({
  ...
  const startQuiz = async () => {
    ...
    try {
      const questionsPerBatch = 1000;
      const result: Question[] = [];

      let remaining = count;
      let offset = Math.floor(
        Math.random() *
          Math.max(1, availableQuestions - count + 1)
      );
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
        ...
        <p className="mb-6 max-w-md text-sm text-slate-500">
          Choose a category and the number of questions you want
          to attempt.
        </p>

        <div className="mb-4 w-full max-w-xs">
          <label className="mb-1.5 block text-left text-xs font-semibold text-slate-600">
            Category
          </label>
        ...
        <div className="mb-6 w-full max-w-xs">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none rounded-lg border border-slate-200 bg-white py-2.5 px-3 pr-9 text-sm font-medium text-slate-700 shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100"
            >
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mb-5 text-base font-semibold leading-relaxed text-slate-900">
          {current.question}
        </p>
          ...
          {current.options.map((opt, i) => {
            ...
            return (
              <button
                ...
                disabled={answered}
                className={`flex w-full items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm font-medium transition ${style} ${
                  !answered
                    ? 'active:scale-[0.99]'
                    : 'cursor-default'
                }`}
...
function QuestionForm({
  ...
  const removeOption = (index: number) => {
    ...
    if (answer >= newOptions.length) {
      setAnswer(newOptions.length - 1);
    }
  };
